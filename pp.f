...
C         fz(i) = 0.
         ...
         if( abs(fs) .lt. fricf ) then
               bt = -fricf*vt(i,tno)/ds
               bb = -fricf*vb(i,tno)/ds
               cn = 2. * sqrt(abs(k1 * (mass(i)+mass(j))/2))
               cs = 2. * sqrt(abs(k1/5. *(mass(i)+mass(j))/2))
         else
            bt = ft(i,tno)*fricf/fs
            bb = fb(i,tno)*fricf/fs
            ft(i,tno) = 0.
            fb(i,tno) = 0.
            cn = 2. * sqrt(abs(k1 * (mass(i)+mass(j))/2))
            cs = 0.
...
C      write(55,*) 'fpn=',fpn,'fpt=',fpt,'fpb=',fpb
C      write(55,*) 'bn=',bn,'cf=',cf,'dvn',dvn
C      write(55,*)
...
**************** van der Waals force *****************
      ...
      dd = d-r(i)-r(j)
        if(dd.lt.4.0e-8) then
          dd=4.0e-8
        endif

      bn = -1.0*aa*r(i)*r(j)/(6.0*(r(i)+r(j))*dd)
...
C      endif
 ...
 999  if (xikeep .ne. 0.0) x(i) = xikeep
      if (xjkeep .ne. 0.0) x(j) = xjkeep
